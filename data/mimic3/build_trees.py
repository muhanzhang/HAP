...
if __name__ == '__main__':
    ...
    for icdCode, ancestors in newFiveMap.iteritems():
        tmp = ancestors[1:] + [ancestors[0]]
        for i in range(5):
            parent, child = tmp[i], tmp[i+1]
            p2c[i][parent].add(child)
            c2p[i][child].add(parent)
    for icdCode, ancestors in newFourMap.iteritems():
        tmp = ancestors[1:] + [ancestors[0]]
        for i in range(4):
            parent, child = tmp[i], tmp[i+1]
            p2c[i][parent].add(child)
            c2p[i][child].add(parent)
    for icdCode, ancestors in newThreeMap.iteritems():
        tmp = ancestors[1:] + [ancestors[0]]
        for i in range(3):
            parent, child = tmp[i], tmp[i+1]
            p2c[i][parent].add(child)
            c2p[i][child].add(parent)
    for icdCode, ancestors in newTwoMap.iteritems():
        tmp = ancestors[1:] + [ancestors[0]]
        for i in range(2):
            parent, child = tmp[i], tmp[i+1]
            p2c[i][parent].add(child)
            c2p[i][child].add(parent)
    for icdCode, ancestors in newOneMap.iteritems():
        tmp = ancestors[1:] + [ancestors[0]]
        for i in range(1):
            parent, child = tmp[i], tmp[i+1]
            p2c[i][parent].add(child)
    ...
    pickle.dump(types, open(outFile + '.types.old', 'wb'), -1)
    ...
    pickle.dump((p2c, c2p), open(outFile + '.p2c', 'wb'), -1)
