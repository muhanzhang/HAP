...
def unzip(zipped):
    ...
    #new_params['W_emb'] = W_emb['W_emb'].eval()
    return new_params
...
def init_params(options):
    ...
    if False:
        options['embDimSize'] = inputDimSize + numAncestors
        params['W_emb'] = np.eye(options['embDimSize'], dtype='float32')
        embDimSize = options['embDimSize']
    ...
    if args.sep_attention:
        params['W_attention2'] = get_random_weight(embDimSize*2, attentionDimSize)
        params['b_attention2'] = np.zeros(attentionDimSize).astype(config.floatX)
        params['v_attention2'] = np.random.uniform(-0.1, 0.1, attentionDimSize).astype(config.floatX)
...
def generate_attention(tparams, W_parents, W_children, mask, second=False):
    attentionInput = T.concatenate([W_parents, W_children], axis=2)
    if not second:
        ...
        mlpOutput = T.tanh(T.dot(attentionInput, tparams['W_attention2']) + tparams['b_attention2']) 
        preAttention = T.dot(mlpOutput, tparams['v_attention2'])
    preAttention_masked = preAttention + mask
    attention = T.nnet.softmax(preAttention_masked)

    #attention = mlpOutput
    #attention = T.exp(preAttention_masked)
    #attention = preAttention
    return attention
...
def build_model(tparams, p2c, c2p, options):
    # forward pass (bottom up message passing from leaves to root)
    parentsList, childrenList, maskList = [], [], []
    for i, p2c_i in enumerate(p2c[::-1][:args.level]):
        ...
        children = np.array(children).astype('int32')
        mask = np.array(mask)
        parents = []
        for p in p2c_i.keys():
            parents.append([p] * max_n_children)
        parents = np.array(parents).astype('int32')
        sharedParents = theano.shared(parents, name='parents'+str(i))
        sharedChildren = theano.shared(children, name='children'+str(i))
        sharedMask = theano.shared(mask, name='mask'+str(i))
        parentsList.append(sharedParents)
        childrenList.append(sharedChildren)
        maskList.append(sharedMask)

    W_emb_temp = tparams['W_emb'] + 0.
    for i, (parents, children, mask) in enumerate(zip(parentsList, childrenList, maskList)):
        W_parents = tparams['W_emb'][parents]
        if i == 0:
            W_children = tparams['W_emb'][children]
        else:
            W_children = W_emb_temp[children]
        tempAttention = generate_attention(tparams, W_parents, W_children, mask)
        tempEmb = (W_children * tempAttention[:,:,None]).sum(axis=1)
        #W_emb_temp[parents[:, 0]] = tempEmb
        W_emb_temp = T.set_subtensor(W_emb_temp[parents[:, 0]], tempEmb)

    # backward pass (top down message passing from root to leaves)
    parentsList, childrenList, maskList = [], [], []
    for i, c2p_i in enumerate(c2p[args.max_level-args.level:]):
    ...
    for parents, children, mask in zip(parentsList, childrenList, maskList):
        W_children, W_parents = W_emb_temp[children], W_emb_temp[parents]
        tempAttention = generate_attention(tparams, W_children, W_parents, mask, second=args.sep_attention)
        tempEmb = (W_parents * tempAttention[:,:,None]).sum(axis=1)
        #W_emb_temp[children[:, 0]] = tempEmb
        W_emb_temp = T.set_subtensor(W_emb_temp[children[:, 0]], tempEmb)


    emb = W_emb_temp[:options['inputDimSize'], :]
    #tparams['final_emb'] = emb
    ...
    if options['L2'] > 0.:
        ...
        if args.sep_attention:
            cost += options['L2'] * ((tparams['W_attention2']**2).sum() + (tparams['v_attention2']**2).sum())
    else:
        cost = cost_noreg
...
def adadelta(tparams, grads, x, y, mask, lengths, cost):
    ...
    if args.fixed_embed:
        param_up = [(p, p + ud) for (k, p), ud in zip(tparams.items(), updir) if k != 'W_emb' and k != 'final_emb']
    else:
        param_up = [(p, p + ud) for p, ud in zip(tparams.values(), updir) if k != 'final_emb']
...
def build_tree(p2cFile):
    p2c, c2p = pickle.load(open(p2cFile, 'rb'))
    # transform set values to list values
    p2c = [dict([(x, [x]+list(y)) for x, y in z.items()]) for z in p2c]
    c2p = [dict([(x, [x]+list(y)) for x, y in z.items()]) for z in c2p]
    return p2c, c2p
...
def train_HAP(
    ...
    if args.test:
        ...
        for key, value in tempParams.iteritems():
            params[key] = value
        tparams = init_tparams(params)
    use_noise, x, y, mask, lengths, cost, cost_noreg, y_hat, emb =  build_model(tparams, p2c, c2p, options)
    ...
    #tparams = dict(tparams.items()+W_emb.items())
...
def parse_arguments(parser):
    ...
    parser.add_argument('--level', type=int, default=5, help='How many levels of hierarchy to use')
    ...
    parser.add_argument('--sep_attention', action='store_true', help='Use separate attention weights for bottom-up and top-down propagations')
...
if __name__ == '__main__':
    ...
    copy(__file__, args.out_dir)
    cmd_input = 'THEANO_FLAGS=device=gpu0 python ' + ' '.join(sys.argv)
